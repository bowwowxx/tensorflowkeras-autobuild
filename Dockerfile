...
RUN conda install -y \
    ...
    seaborn \
    pillow \
  && conda install -y -c conda-forge tifffile=0.10.0 \
  && conda clean --yes --tarballs --packages --source-cache \
  && pip install --upgrade -I setuptools \
  && pip install --upgrade pip \
  && pip install --upgrade keras https://storage.googleapis.com/tensorflow/linux/cpu/tensorflow-0.9.0rc0-cp27-none-linux_x86_64.whl \
  && pip install tensorflow --upgrade

RUN mkdir /deep-models
ADD deep-models /deep-models
